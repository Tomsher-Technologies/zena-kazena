        ...
        <div class="container container--type-2">
            ...
            <div class="collection__top-filter clearfix">
                ...
                <div class="collection__founded-products collection__founded-products--type-2">
                    ...
                    @else
                        <h1 class="shop-list-name">{{ trans('messages.items_for_auction') }}</h1>
                    @endif
                    ...
                    <div class="top-filter top-filter--left-column js-top-filter">
                        ...
                        <div class="top-filter__background">
                            ...
                            <div class="top-filter__clo">
                                <a href="{{ route('auction.products') }}" ><i class="lnil lnil-close"></i> {{ trans('messages.clear_filter') }}</a>
                            </div>
                            <!-- End close --->
                            <!-- Row -->
                                                ...
                                                <ul class="widget__collections">
                                                    @foreach ($categories as $cat)
                                                        <!-- Collections -->
                                                            ...
                                                            <li><a class="@if($category == $cat->getTranslation('slug', $lang)) active-category @endif" href="{{ route('auction.products', ['category' => $cat->getTranslation('slug', $lang)]) }}">{{ $cat->getTranslation('name', $lang)}}</a></li>
                                    ...
                                    <div class="product-grid-item product-grid-item--type-2 product-grid-item--type-5">
                                        ...
                                        <div class="product-grid-item__images product-grid-item__images--ratio-100-122 js-product-grid-images"
                                            ...
                                            <div class="product-grid-item__quick-shop">
                                                <a href="{{ route('auction.product-detail',['slug' => $prod->slug, 'sku' => $prod->sku]) }}" class="">{{ trans('messages.view') }} {{ trans('messages.details') }}</a>
                                            </div>
                                            <!-- End quick shop -->
                                        </div>
                                        <!-- End product images -->
                                        ...
                                        <div class="product-grid-item__name">
                                            <a href="{{ route('auction.product-detail',['slug' => $prod->slug, 'sku' => $prod->sku]) }}">{{ $prod->getTranslation('name', $lang) }}</a>
                                        </div>
                                        <!-- End product name -->
                                        ...
                                        <div class="product-grid-item__price">
                                            <!-- Price new -->
                                            @php
                                                if ($prod->stocks[0]?->price > $prod->stocks[0]?->high_bid_amount){
                                                    $price = $prod->stocks[0]?->price;
                                                }else {
                                                    $price = $prod->stocks[0]?->high_bid_amount;
                                                }
                                            @endphp
                                            
                                            <span class="product-grid-item__price-new"> {{ env('DEFAULT_CURRENCY').' '.$price }}</span>
                                            
                                            @if ($prod->stocks[0]?->price != $prod->stocks[0]?->high_bid_amount && $prod->stocks[0]?->high_bid_amount != 0)
                                                <span class="product-grid-item__price-old">{{ env('DEFAULT_CURRENCY').' '.$prod->stocks[0]?->price }}</span>
                                            @endif
                                            <!-- Price old -->
